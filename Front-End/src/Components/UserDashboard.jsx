...
function UserDashboard() {
  ...
  useEffect(() => {
    const fetchEntregas = async () => {
      try {
        const response = await axios.get(`https://gerenciamento-de-entregas.onrender.com/entregas/usuario/${userId}`, {
          headers: {
  ...
  const handleStatusChange = async (entregaId) => {
    try {
      ...
      const response = await axios.put(
        `https://gerenciamento-de-entregas.onrender.com/entregas/${entregaId}/atualizar-status`,
        entregaDTO,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
